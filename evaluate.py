...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    for i in range(max_len-1):                                                    # 为什么要max_len减去1
        out = model.decode(memory, src_mask, 
                           Variable(ys), 
                           Variable(subsequent_mask(ys.size(1))
                                    .type_as(src.data)))
        prob = model.generator(out[:, -1])                              # 查看out是几维度的，方可知输出维度
        _, next_word = torch.max(prob, dim=1)                           # next_word：接收的是最大值，_接收的是dim维度上的索引
        next_word = next_word.data[0]                                   # 第0层
        ys = torch.cat([ys, 
